                    ...
                    <form role="form" action="<?= $url_action ?>" method="post" class="form-horizontal">
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-md-8" id="client">
                                ...
                                <select value="" class="form-control" type="" id="client_id" name="client_id" <?= ((($show)) ? 'disabled' : '') ?>>
                                    <?php foreach ($clients as $each): ?>
                                        <option <?= (($each->id == $instance->client_id) ? 'selected' : '') ?> value="<?= $each->id ?>"><?= $each->name ?></option>
                                    <?php endforeach; ?>
                                </select>
                            </div>
                        </div>
                        <div class="form-group">
                            ...
                            <div class="col-md-8">
                                <select data-name="<?= $instance->parent_id ?>" value="" class="form-control" type="" id="parent_id" name="parent_id" <?= ((($show)) ? 'disabled' : '') ?> >
                                    <option value="">N/A</option>
                                    <?php foreach ($projects as $each): if ($each->id == $instance->id) continue; ?>
                                        <option <?= (($each->id == $instance->parent_id) ? 'selected' : '') ?>  value="<?= $each->id ?>"><?= $each->name ?></option>
                                    <?php endforeach; ?>
                                </select>
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-md-8">
                                <textarea name="description" cols="40" rows="10" id="description" class="form-control" maxlength="" <?= ((($show)) ? 'disabled' : '') ?> <?= (($instance->active == 't') ? 'checked' : '') ?>><?=$instance->description?></textarea>
                            </div>
                        </div>
                        <br>
                        <?php if (($show)): ?>
